...
class VCTradeFavorites: VCBase ,UITableViewDataSource, UITableViewDelegate , FirebaseInterface, ValueEventListener{
    enum COINSORT{
        case NORMAL
        case UP
        case DOWN
        ...
        if let newarray = appInfo.getCoinList(){
            for coin in newarray{
                if let _ = coin.myCoinData{
                    if let attention = coin.myCoinData?["attention_coin_BTC"] as? String{
                        if attention == "Y" {
                            coins.append(coin)
                        }
                    }
